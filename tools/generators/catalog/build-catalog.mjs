 ...
 * Catalog generator with per-style directory structure
 */
...
(async function main() {
  const {
    docsCatalogDir, schemaRel
  } = resolvePaths( {
      repoRoot
    });
  await fs.mkdir(docsCatalogDir, {
    recursive: true
  });
  ...
  try {
    ...
    console.warn(
      "[build-catalog] alert: versions.json not found or invalid, defaulting to 0.0.0"
    );
  }
  ...
  for (const {
    ...
    const styleCatalogDir = path.join(docsCatalogDir, style);
    await fs.mkdir(styleCatalogDir, {
      recursive: true
    });
    
    for (const variant of variants) {
      let entries = await loadIconsMeta( {
      ...
      (versions.core && versions.core.version) ||
      "0.0.0";
